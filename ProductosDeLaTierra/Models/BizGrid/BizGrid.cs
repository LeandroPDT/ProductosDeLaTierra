...
namespace System.Web.Helpers {
    public class BizGrid {
        ...
        private const string IncrementKey = "__BizGridIncrement";
        ...
        // NOTE: BizGrid uses an IEnumerable<dynamic> data source instead of IEnumerable<T> to avoid generics in the syntax.
        internal BizGrid(
            ...
            if (rowsPerPage < 1) {
                throw new ArgumentOutOfRangeException("rowsPerPage", String.Format(CultureInfo.CurrentCulture,
                    "Filas por página debe ser mayor a 0", 1));
            }
        ...
        public int PageIndex {
            ...
            set {
                if (!_canPage) {
                    throw new NotSupportedException("No suportado porque no se puede paginar");
                }
                ...
                if (!_dataSourceBound) {
                    ...
                    if (value < 0) {
                        ...
                        "Argument_Must_Be_GreaterThanOrEqualTo", 0));
                    }
                    else {
                        _pageIndex = value;
                        _pageIndexSet = true;
                    ...
                    if ((value < 0) || (value >= PageCount)) {
                        throw new ArgumentOutOfRangeException("value", String.Format(CultureInfo.CurrentCulture,
                            "Argument_Must_Be_Between", 0, (PageCount - 1)));
                    }
        ...
        public BizGrid Bind(IEnumerable<dynamic> source, IEnumerable<string> columnNames = null, bool autoSortAndPage = true, int rowCount = -1) {
            if (_dataSourceBound) {
                throw new InvalidOperationException("BizGrid_DataSourceBound");
            }
            if (source == null) {
                throw new ArgumentNullException("source");
            }
            if (!autoSortAndPage && _canPage && rowCount == -1) {
                throw new ArgumentException("BizGrid_RowCountNotSpecified", "rowCount");
            }
            ...
            if (!autoSortAndPage) {
                _dataSource = new PreComputedBizGridDataSource(grid: this, values: source, totalRows: rowCount);
            }
            else {
                BizGridDataSource dataSource = new BizGridDataSource(grid: this, values: source, elementType: _elementType, canPage: _canPage, canSort: _canSort);
        ...
        public BizGridColumn Column(string columnName = null, string header = null, Func<dynamic, object> format = null, string style = null,
            bool canSort = true, string summaryOperation = "", bool isHeaderCheckbox = false) {

            if (String.IsNullOrEmpty(columnName)) {
                if (format == null) {
                    throw new ArgumentException("BizGrid_ColumnNameOrFormatRequired", "columnName");
                }
            }

            return new BizGridColumn() { ColumnName = columnName, Header = header, Format = format, Style = style, CanSort = canSort, SummaryOperation = summaryOperation, IsHeaderCheckbox = isHeaderCheckbox };
        }

        // Should we keep this no-op API for improved BizGrid syntax? Alternatives are:
        ...
        public BizGridColumn[] Columns(params BizGridColumn[] columnSet) {
            return columnSet;
        }
        ...
        public IHtmlString GetHtml(
            ...
            IEnumerable<BizGridColumn> columns = null,
            IEnumerable<string> exclusions = null,
            BizGridPagerModes mode = BizGridPagerModes.NextPrevious | BizGridPagerModes.Numeric,
        ...
        public string GetPageUrl(int pageIndex) {
            if (!_canPage) {
                throw new NotSupportedException("BizGrid_NotSupportedIfPagingIsDisabled");
            }
        ...
        public string GetSortUrl(string column) {
            if (!_canSort) {
                throw new NotSupportedException("BizGrid_NotSupportedIfSortingIsDisabled");
            }
            if (String.IsNullOrEmpty(column)) {
                throw new ArgumentException("Argument_Cannot_Be_Null_Or_Empty", "column");
            }
        ...
        public HelperResult Pager(
            ...
            if (!ModeEnabled(mode, BizGridPagerModes.FirstLast) && (firstText != null)) {
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture,
                    "BizGrid_PagerModeMustBeEnabled", "FirstLast"), "firstText");
            }
            if (!ModeEnabled(mode, BizGridPagerModes.NextPrevious) && (previousText != null)) {
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture,
                    "BizGrid_PagerModeMustBeEnabled", "NextPrevious"), "previousText");
            }
            if (!ModeEnabled(mode, BizGridPagerModes.NextPrevious) && (nextText != null)) {
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture,
                    "BizGrid_PagerModeMustBeEnabled", "NextPrevious"), "nextText");
            }
            if (!ModeEnabled(mode, BizGridPagerModes.FirstLast) && (lastText != null)) {
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture,
                    "BizGrid_PagerModeMustBeEnabled", "FirstLast"), "lastText");
            }
            ...
            return new HelperResult(tw => {
                ...
                if (ModeEnabled(mode, BizGridPagerModes.FirstLast) && currentPage > 1) {
                ...
                if (ModeEnabled(mode, BizGridPagerModes.NextPrevious) && currentPage > 0) {
                ...
                if (ModeEnabled(mode, BizGridPagerModes.Numeric) && (totalPages > 1)) {
                ...
                if (ModeEnabled(mode, BizGridPagerModes.NextPrevious) && (currentPage < lastPage)) {
                ...
                if (ModeEnabled(mode, BizGridPagerModes.FirstLast) && (currentPage < lastPage - 1)) {
        ...
        public IHtmlString Table(
            ...
            // In order of precedence, the parameters that affect the visibility of columns in BizGrid - 
            ...
            if (!String.IsNullOrEmpty(tableStyle)) {
                table.MergeAttribute("class", tableStyle + " nicetable filtrable sortable");
            }
            table.MergeAttribute("class", "nicetable filtrable sortable");

            if (htmlAttributes != null) {
                var dictionary = ObjectToDictionary(htmlAttributes);
            ...
            if (displayHeader) {
                ...
                if (_filtrable) { 
                    thead.InnerHtml += string.Format(@"<tr>
                                    <td colspan='{0}' class='filtrablerow'>
                                        <span class='quicksearchicon'></span>
                                        <input class='quicksearch' type='text' />
                                    </td>
                                </tr>", columns.Count().ToString());
                }

                if (!string.IsNullOrEmpty(newButtonText)) {
                    thead.InnerHtml += string.Format(@"<tr><td colspan='{0}' class='newbuttonrow'>{1}</td></tr>", columns.Count().ToString(), newButtonText);
                }
            ...
            if (footer != null) {
                ...
                table.InnerHtml += GetTableFooterHtml(columns, footerStyle);
            }
        ...
        private void EnsureColumnIsSortable(IEnumerable<BizGridColumn> columns) {
            ...
            if (_canSort && !_sortColumnExplicitlySet && !String.IsNullOrEmpty(SortColumn) && !StringComparer.OrdinalIgnoreCase.Equals(_defaultSort, SortColumn)
                ...
                throw new InvalidOperationException(String.Format(CultureInfo.CurrentCulture, "BizGrid_ColumnNotFound", SortColumn));
            }
        }

        internal static dynamic GetMember(BizGridRow row, string name) {
            object result;
        ...
        private void EnsureDataBound() {
            if (!_dataSourceBound) {
                throw new InvalidOperationException("BizGrid_NoDataSourceBound");
            }
        }

        private void EnsureDataSourceNotMaterialized() {
            if (_dataSourceMaterialized) {
                throw new InvalidOperationException("izGrid_PropertySetterNotSupportedAfterDataBound");
            }
        }
            ...
            if (!String.IsNullOrEmpty(AjaxUpdateContainerId)) {
                if (!_jqueryChecked) {
                    ...
                    scriptTag.InnerHtml = String.Format(CultureInfo.CurrentCulture,
                        "if (typeof(jQuery)=='undefined') alert(\"{0}\");", HttpUtility.JavaScriptStringEncode("BizGrid_JScriptReferenceRequired"));
                    _jqueryChecked = true;
        ...
        private IEnumerable<BizGridColumn> GetDefaultColumns(IEnumerable<string> exclusions) {
            IEnumerable<string> names = ColumnNames;
            if (exclusions != null) {
                names = names.Except(exclusions);
            }
            return (from n in names select new BizGridColumn() { ColumnName = n, CanSort = true }).ToArray();
        }
        ...
        private string GetTableBodyHtml(IEnumerable<BizGridColumn> columns, string rowStyle,
            ...
            foreach (var row in Rows) {
                ...
                foreach (var column in columns) {
                    var value = (column.Format == null) ?
                        StandardFormat(column, row[column.ColumnName]) : Format(column.Format, row).ToString();
        ...
        private static string GetTableCellHtml(BizGridColumn column, string innerHtml, string extraClass = "") {
            TagBuilder td = new TagBuilder("td");
            if (!String.IsNullOrEmpty(column.Style) || (!String.IsNullOrEmpty(column.StandardStyle))) {
                td.MergeAttribute("class", column.Style + " " + column.StandardStyle + " " + extraClass);
            }
        ...
        private static string StandardFormat(BizGridColumn column, object Valor) {
            ...
            if (Valor.GetType() == typeof(decimal) || Valor.GetType() == typeof(float)) {
                ...
                else if (column.Style == "time" || column.Style == "hora") {
                    return ((DateTime)Valor).ToShortTimeString();
                }
        ...
        private string GetTableFillerRowsHtml(IEnumerable<BizGridColumn> columns, string rowStyle,
        ...
        private string GetTableFooterHtml(IEnumerable<BizGridColumn> columns, string footerStyle) {
            ...
            int SpanCount = 0;
            foreach (var column in columns) {
                if (!column.SummaryOperation.IsEmpty()) {
                    if (SpanCount > 0) {
                        TagBuilder td = new TagBuilder("td");
                        td.MergeAttribute("colspan", SpanCount.ToString(CultureInfo.InvariantCulture));
                        tr.InnerHtml += td.ToString();
                    }
                    decimal Valor = 0; string cellText = ""; 
                    foreach (var row in Rows) {
                        Valor += Convert.ToDecimal(row[column.ColumnName] ?? 0m);
	                }
                    // al total no le puedo aplicar el format 
                    //cellText = (column.Format == null) ? StandardFormat(column, Valor) : Format(column.Format, Valor).ToString();
                    cellText = StandardFormat(column, Valor);
                    tr.InnerHtml += GetTableCellHtml(column, cellText, "summary" + column.ColumnName);
                    SpanCount = 0;
                }
                else {
                    SpanCount++;
        ...
        private string GetTableHeaderHtml(IEnumerable<BizGridColumn> columns, string headerStyle) {
            TagBuilder tr = new TagBuilder("tr");
            ...
            foreach (var column in columns) {
                ...
                if (column.IsHeaderCheckbox) {
                    th.MergeAttribute("class", "sorter-false");
                    th.InnerHtml = "<input type='checkbox' name='checkboxall' id='checkboxall' />"; // implementacion de la funciones queda totalmente abierta a quien llama
                }
                else if (!_canSort || headerIsEmpty || String.IsNullOrEmpty(column.ColumnName) || !column.CanSort) {
        ...
        private static bool ModeEnabled(BizGridPagerModes mode, BizGridPagerModes modeCheck) {
        ...
        internal static IDictionary<string, object> ObjectToDictionary(object instance) {
            ...
            if (instance != null) {
                foreach (System.ComponentModel.PropertyDescriptor descriptor in System.ComponentModel.TypeDescriptor.GetProperties(instance)) {
