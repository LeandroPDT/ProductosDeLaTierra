...
namespace Site.Models {
    public static class Utils {
        public static int GetMonthDiff(DateTime MinDate, DateTime MaxDate) {
            return 12 * (MaxDate.Year - MinDate.Year) + MaxDate.Month - MinDate.Month;
        }
        public static string TextOverflow(string Texto, int MaxChars, string EllipsisToShow = "...") {
        ...
        public static decimal DateDiferenciaMinutos(DateTime HoraDesde, DateTime HoraHasta) {
            // le saco por las dudas la parte de la fecha
            var Desde = HoraDesde.TimeOfDay;
            var Hasta = HoraHasta.TimeOfDay;

            if (Desde == Hasta) {
                return 0;
            }
            else if (Desde < Hasta) {
                return (decimal)(Hasta - Desde).TotalMinutes;
            }
            else {
                // divido la parte antes de la medianoche y despues de la medianoche
                decimal PrimerParcial = (decimal)(new TimeSpan(23, 59, 0) - Desde).TotalMinutes + 1;
                return Math.Round(PrimerParcial + (decimal)(Hasta - new TimeSpan(0, 0, 0)).TotalMinutes, 2);
            }
        }
        public static bool DatePeriodoHoraEn(DateTime HDesde, DateTime HHasta, DateTime HInicio, DateTime HFin) {
            return DateHoraEntre(HDesde, HInicio, HFin) || DateHoraEntre(HHasta, HInicio, HFin) || (HDesde == HInicio && HHasta == HFin);
        }
        public static bool DateHoraEntre(DateTime LaHora, DateTime LaHoraDesde, DateTime LaHoraHasta) {
            // le saco por las dudas la parte de la fecha
            var Desde = LaHoraDesde.TimeOfDay;
            var Hasta = LaHoraHasta.TimeOfDay;
            var Hora = LaHora.TimeOfDay;

            if (Desde == Hasta) {
                return false;
            }
            else if (Desde < Hasta) {
                return (Hasta - Hora).TotalMinutes > 0 && (Hora - Desde).TotalMinutes > 0;
            }
            else {
                // divido la parte antes de la medianoche y despues de la medianoche
                return DateHoraEntre(LaHora, LaHoraDesde, new DateTime(2000,1,1, 23,59,59)) || DateHoraEntre(LaHora, new DateTime(2000,1,1,0,0,0), LaHoraHasta);
            }

        }

        public static int CUITGetDigitoVerificador(string CUIT) {
            if (CUIT != null && CUIT.Length >= 10 ) {
                var Suma = 0;
                Suma += CUIT.Substring(0, 1).TryCInt() * 5;
                Suma += CUIT.Substring(1, 1).TryCInt() * 4;
                Suma += CUIT.Substring(2, 1).TryCInt() * 3;
                Suma += CUIT.Substring(3, 1).TryCInt() * 2;
                Suma += CUIT.Substring(4, 1).TryCInt() * 7;
                Suma += CUIT.Substring(5, 1).TryCInt() * 6;
                Suma += CUIT.Substring(6, 1).TryCInt() * 5;
                Suma += CUIT.Substring(7, 1).TryCInt() * 4;
                Suma += CUIT.Substring(8, 1).TryCInt() * 3;
                Suma += CUIT.Substring(9, 1).TryCInt() * 2;
                var Resto = Suma % 11;
                var Digito = 11 - Resto;
                Digito = (Digito == 11 ? 0 : Digito);
                Digito = (Digito == 10 ? 1 : Digito);
        ...
        public static bool CUITValid(string CUIT) {
            return CUIT != null && CUIT.Length == 11 && CUITGetDigitoVerificador(CUIT) == CUIT.Substring(10, 1).TryCInt();
        }

        public static SelectList GetSelectListOf<T>(T ValorActualSeleccionado) {
            Type t = typeof(T);
            if (t.IsEnum) {
                var Lista = (from Enum e in Enum.GetValues(t)
                             select new { ID = e, Nombre = Enumerations.GetEnumDescription(e) }).ToList();

                return new SelectList(Lista, "ID", "Nombre", ValorActualSeleccionado);
            }
            return null;
        }

        // no se puede innovar. Todos los ensayos andan con la funcion anterior a esta
        //public static SelectList GetSelectListOf<T>(T ValorActualSeleccionado, string IncludeAllText = "") {
        //    Type t = typeof(T);
        //    if (t.IsEnum) {
        //        var Lista = (from Enum e in Enum.GetValues(t)
        //                     select new IDNombrePar { ID = Convert.ToInt32((object)e), Nombre = Enumerations.GetEnumDescription(e) }).ToList();

        //        if (!String.IsNullOrEmpty(IncludeAllText)) {
        //            Lista.Insert(0, new IDNombrePar { ID = 0, Nombre = IncludeAllText });
        //        }
        //        return new SelectList(Lista, "ID", "Nombre", ValorActualSeleccionado);
        //    }
        //    return null;
        ...
        public static SelectList GetSelectListOf<T>(int ValorActualSeleccionado) {
            Type t = typeof(T);
            if (t.IsEnum) {
                var Lista = (from Enum e in Enum.GetValues(t)
                             select new { ID = Convert.ToInt32((object)e), Nombre = Enumerations.GetEnumDescription(e) }).ToList();
        ...
        public static SelectList GetSelectListOf<T>(string ValorActualSeleccionado) {
            Type t = typeof(T);
            if (t.IsEnum) {
                var Lista = (from Enum e in Enum.GetValues(t)
                             select new { ID = Convert.ToString((object)e), Nombre = Enumerations.GetEnumDescription(e) }).ToList();
        ...
        public static decimal CeroDiv(decimal Dividendo, decimal Divisor) {
            return (Divisor != 0 ? Dividendo / Divisor : 0);
        }

        public static string Unir(string Separador, params string[] Terminos) {
            string retval = "";
            for (int i = 0; i < Terminos.Length; i++) {
                if (!string.IsNullOrEmpty(Terminos[i])) {
                    retval += (string.IsNullOrEmpty(retval) ? "" : Separador) + Terminos[i];
                }
            }
            return retval;
        }
        ...
        public static long DateDiff(DateInterval interval, DateTime FechaMenor, DateTime FechaMayor, DayOfWeek eFirstDayOfWeek = DayOfWeek.Sunday) {
            if (interval == DateInterval.Year)
                return FechaMayor.Year - FechaMenor.Year;

            if (interval == DateInterval.Month)
                return (FechaMayor.Month - FechaMenor.Month) + (12 * (FechaMayor.Year - FechaMenor.Year));
            ...
            if (interval == DateInterval.WeekOfYear) {
                while (FechaMayor.DayOfWeek != eFirstDayOfWeek)
                    FechaMayor = FechaMayor.AddDays(-1);
                while (FechaMenor.DayOfWeek != eFirstDayOfWeek)
                    FechaMenor = FechaMenor.AddDays(-1);
                ts = FechaMayor - FechaMenor;
                return Round(ts.TotalDays / 7.0);
            }

            if (interval == DateInterval.Quarter) {
                double d1Quarter = GetQuarter(FechaMenor.Month);
                double d2Quarter = GetQuarter(FechaMayor.Month);
                double d1 = d2Quarter - d1Quarter;
                double d2 = (4 * (FechaMayor.Year - FechaMenor.Year));
                return Round(d1 + d2);
            }
        ...
        public static string ToShortDateString(DateTime fecha) {
            return fecha.ToShortDateString();
        }
        public static string ToShortDateString(DateTime? fecha) {
            return fecha.ToShortDateString();
        }
        public static HtmlString PrettyDiff(string texto1, string texto2) {
            var dmp = new DiffMatchPatch.diff_match_patch();
            var d = dmp.diff_main(texto1 ?? "", texto2 ?? "");
            dmp.diff_cleanupSemantic(d);
            return new HtmlString( dmp.diff_prettyHtml(d));
            
        }

        public static SelectList SelectListWith(SelectListItem ElItem) {
        ...
        public static string SaveAsThumbnailCropped(string path, string archivo, int NewWidth, int NewHeight) {

            // Declare two variables of type Integer named
            //    adjustedImageWidth and adjustedImageHeight.
            int adjustedImageWidth = 0;
            int adjustedImageHeight = 0;
            decimal ratio = 0;

            // Declare a variable named theImage of type Image.
            Image theImage = default(Image);
            string ThumbFileName = null;
            string extension = Path.GetExtension(archivo).ToLower();
            string nombre = Path.GetFileNameWithoutExtension(archivo).ToLower();
            ThumbFileName = nombre + "_tb" + NewWidth.ToString() + "x" + NewHeight.ToString() + extension;

            if (!File.Exists(path + "\\" + ThumbFileName) && File.Exists(path + "\\" + archivo)) {
                ...
                if (theImage.Height > NewHeight || theImage.Width > NewWidth) {
                    if (theImage.Height * NewWidth / theImage.Width > NewHeight) {
                        // tengo que reducir el alto
                        adjustedImageHeight = NewHeight;
                        //keep ratio
                        ratio = Utils.CeroDiv(adjustedImageHeight, theImage.Height);
                        adjustedImageWidth = (int)Math.Round((decimal)theImage.Width * ratio, 0);
                    }
                    else {
                        adjustedImageWidth = NewWidth;
                        //keep ratio
                        ratio = Utils.CeroDiv(adjustedImageWidth, theImage.Width);
                        adjustedImageHeight = (int)Math.Round((decimal)theImage.Height * ratio, 0);
                    }
                }
                else {
                ...
                if (adjustedImageHeight < NewHeight || adjustedImageWidth > NewWidth) {
                    //era muy apaisada tengo que croppear el centro
                    adjustedImageHeight = NewHeight;
                    ratio = Utils.CeroDiv(adjustedImageHeight, theImage.Height);
                    adjustedImageWidth = (int)Math.Round((decimal)theImage.Width * ratio, 0);
                    var WidthSobrante = adjustedImageWidth - NewWidth;
                    cropRect = new Rectangle(WidthSobrante / 2, 0, NewWidth, NewHeight);
                }
                else if (adjustedImageHeight > NewHeight || adjustedImageWidth < NewWidth) {
                    adjustedImageWidth = NewWidth;
                    ratio = Utils.CeroDiv(adjustedImageWidth, theImage.Width);
                    adjustedImageHeight = (int)Math.Round((decimal)theImage.Height * ratio, 0);
                    //quedo muy larga. Le cropeo el 25% de arriba del sobrante
                    var HeightSobrante = adjustedImageHeight - NewHeight;
                ...
                System.Drawing.Image Image = theImage;
                System.Drawing.Image thumbnail = new Bitmap(adjustedImageWidth, adjustedImageHeight);
                ...
                var croppedThumbnail = cropImage(thumbnail, cropRect);


                if (extension.Equals(".gif")) {
                    ImageQuantization.OctreeQuantizer quantizer = new ImageQuantization.OctreeQuantizer(255, 8);
                    Bitmap quantized = quantizer.Quantize(croppedThumbnail);
                    ...
                    ImageCodecInfo jpgCodec = default(ImageCodecInfo);
                    jpgCodec = GetImageCodec("image/jpeg");

                    if (jpgCodec != null) {
                        //Create quality parameter
                        EncoderParameter qParam = new EncoderParameter(System.Drawing.Imaging.Encoder.Quality, 100L);
        ...
        private static Image cropImage(Image img, Rectangle cropArea) {
            dynamic bmpImage = new Bitmap(img);
            dynamic bmpCrop = bmpImage.Clone(cropArea, bmpImage.PixelFormat);
        ...
        public static int randomNumber(int desde, int hasta) {
            ...
            return generator.Next(desde, hasta);
        }
        ...
        public static string randomLetter() {
            ...
            return ((char)generator.Next((int)'a', (int)'z')).ToString();
        }

        public static string JoinNonEmpty(string Separator, string Texto1, string Texto2) {
            if (string.IsNullOrEmpty(Texto1)) return Texto2;
            if (string.IsNullOrEmpty(Texto2)) return Texto1;
            return Texto1 + Separator + Texto2;
        }


        public static string randomLetters(int CantidadDeLetras) {
            var retval = new System.Text.StringBuilder();
            for (int i = 1; i <= CantidadDeLetras; i++) {
                retval.Append(randomLetter());
            }
